...
const Projects = () => {
  ...
  const [cardsPerView, setCardsPerView] = useState(3); // ✅ added
  const cardRef = useRef(null);
  const { ref: headingRef, inView: headingInView } = useInView({ threshold: 0.2 });

  const totalCards = projects.length;

  // ✅ updated useEffect (only this section)
  useEffect(() => {
    const updateWidth = () => {
      if (cardRef.current) {
  ...
  return (
    <section id="projects" className="bg-black text-white h-screen flex flex-col justify-center py-6 relative">
      <div className="container mx-auto px-8 md:px-16 lg:px-24">
        ...
        >
          My <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-green-400">Projects</span>
        </motion.h2>

        <p className="text-center text-gray-400 mb-6 text-xs md:text-sm">Practical projects showcasing development & design experience.</p>
              ...
              {projects.map((project, index) => (
                ...
                >
                  <a href={project.link} target="_blank" className="block mb-3">
                    <motion.img src={project.image} className="w-full h-44 object-contain rounded-lg bg-black p-2" whileHover={{ scale: 1.07 }} />
                  </a>

                  <h3 className="text-sm font-semibold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-green-400 mb-1">{project.name}</h3>
                  <p className="text-gray-400 text-xs mb-2">{project.tech}</p>

                  <a href={project.link} target="_blank" className="inline-flex items-center text-white hover:text-green-400 text-lg transition-all duration-300">
        ...
        {/* ✅ Matching Neon Navigation Buttons */}
        <button
          onClick={() => currentIndex > 0 && setCurrentIndex(currentIndex - 1)}
          className={`
            absolute left-8 top-1/2 -translate-y-1/2 
            w-10 h-10 flex items-center justify-center 
            rounded-full bg-black border border-green-400/50
            shadow-[0_0_12px_rgba(0,255,170,0.5)]
            hover:shadow-[0_0_18px_rgba(0,255,170,0.8)]
